	 ...
	 */
	 private double referencesRemoved(Node original, Node version) {
		...
		if (!targetClassInVerGraph.hasCallGraph()) {
					...
					createClassReferenceGraph(targetClassInVerGraph, graph2, new NullProgressMonitor());
			targetClassInVerGraph.setCreatedCallGraph();
		}
		if (!targetClassInOrigGraph.hasCallGraph()) {
			 
					createClassReferenceGraph(targetClassInOrigGraph, graph1, new NullProgressMonitor());
			targetClassInOrigGraph.setCreatedCallGraph();
		}

		List originalClassReferences = 
				graph1.getAllEdges(original, targetClassInOrigGraph);
		List versionClassReferences = 
				graph2.getAllEdges(version, targetClassInVerGraph);
		if (originalClassReferences.size() == 0) {
			if (Flags.isStatic(original.getFlags()))
				return 1.0;
			if (
					isTargetClassAFieldInSourceClass(original, targetClassInOrigGraph))
				return 1.0;
			...
			if (sourceClass != null && destinationClass != null) {
				if ( ClassDetection
					...
					addToPDM.add(pair);
						...
						ClassDetection.isSuperClassOf(destinationClass, sourceClass))
					addToPUM.add(pair);
			}
		}
