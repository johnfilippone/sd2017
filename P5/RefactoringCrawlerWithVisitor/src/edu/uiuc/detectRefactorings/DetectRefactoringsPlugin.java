...
public class DetectRefactoringsPlugin extends AbstractUIPlugin {
	...
	// The shared instance.
	 static private DetectRefactoringsPlugin plugin;
	...
	public void launch(final Dictionary settings) {
		...
		try {
			...
			Throwable targer = e.getTargetException();
					 ...
					 ErrorDialog.openError(getShell(), "TargetError", "Error Occured While Running Detections", new Status(0,
...
//		AbstractBaseGraph versionGraph = navigatorForVersion.getGraph();
		...
		if (((Boolean) settings.get("UseFeedbackLoop")).booleanValue()) {
					...
					doFeedbackLoop(monitor, shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		...
		if (((Boolean) settings.get("RenameMethod")).booleanValue()) {
					 ...
					 detectRenameMethod(monitor, tRenameMethod, shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		}

		if (((Boolean) settings.get("RenameClass")).booleanValue()) {
			   
					 detectRenameClass(monitor, tRenameClass, shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		}

		if (((Boolean) settings.get("RenamePackage")).booleanValue()) {
			   
					 detectRenamePackage(monitor, tRenamePackage, shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		}

		if (((Boolean) settings.get("MoveMethod")).booleanValue()) {
			   
					 detectMoveMethod(monitor, tMoveMethod, shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		}

		if (((Boolean) settings.get("PullUpMethod")).booleanValue()) {
			  
					  detectPullUpMethod(monitor, tPullUpMethod, shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		}

		if (((Boolean) settings.get("PushDownMethod")).booleanValue()) {
			  
					  detectPushDownMethod(monitor, tPushDownMethod, shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		}
...
//		}
		...
		if (((Boolean) settings.get("ChangeMethodSignature")).booleanValue()) {
					...
					detectChangeMethodSignature(monitor, tChangeMethodSignature, shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		}
...
//		}
	...
	public void doFeedbackLoop(IProgressMonitor monitor,
		...
		// }
				...
				detectRenamePackage(monitor, shinglesUtil.getRenamePackage_treshold(), shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		 
				  
				detectRenameClass(monitor, shinglesUtil.getRenameClass_treshold(), shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		 
				  
				detectRenameMethod(monitor, shinglesUtil.getRenameMethod_treshold(), shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);

		 
				  
				detectPullUpMethod(monitor, shinglesUtil.getPullUp_treshold(), shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		 
				  
				detectPushDownMethod(monitor, shinglesUtil.getPushDown_treshold(), shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
		 
				  
				detectMoveMethod(monitor, shinglesUtil.getMoveMethod_treshold(), shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);

		  
				  detectChangeMethodSignature(monitor, shinglesUtil
				.getChangeMethodSignature_treshold(), shinglesUtil, originalGraph, versionGraph, unitOfWorkPerRefactoring);
	...
	public void detectChangeMethodSignature(IProgressMonitor monitor,
		...
		List changedMethodSignatures = 
				 
				detector.detectRefactorings(candidateChangedMethodSignatures, monitor, unitOfWorkPerRefactoring);
	...
	public void detectMoveField(IProgressMonitor monitor, double tMoveField,
		...
		List moveFieldResults = 
				  detector.detectRefactorings(candidateMovedFields, monitor, unitOfWorkPerRefactoring);
	...
	public void detectMoveMethod(IProgressMonitor monitor, double tMoveMethod,
		...
		List movedMethods = 
				 detector.detectRefactorings(methodCandidates, monitor, unitOfWorkPerRefactoring);
	...
	public void detectMoveClass(IProgressMonitor monitor, double tMoveClass,
		...
		List movedClasses = 
				 detector.detectRefactorings(classCandidates, monitor, unitOfWorkPerRefactoring);
	...
	public void detectPushDownMethod(IProgressMonitor monitor,
		...
		List pushDownMethodResults = 
				  detector.detectRefactorings(candidatePushDownMethods, monitor, unitOfWorkPerRefactoring);
	...
	public void detectPullUpMethod(IProgressMonitor monitor,
		...
		List pullUpMethodResults = 
				  detector.detectRefactorings(candidatePullUpMethods, monitor, unitOfWorkPerRefactoring);
	 ...
	 private void detectRenameMethod(IProgressMonitor monitor, double tMethod,
		...
		List renamedMethods = 
				 detector.detectRefactorings(candidateMethods, monitor, unitOfWorkPerRefactoring);
	...
	public void detectRenameClass(IProgressMonitor monitor, double tClass,
		...
		List renamedClasses = 
				 detector.detectRefactorings(candidateClasses, monitor, unitOfWorkPerRefactoring);
	...
	public void detectRenamePackage(IProgressMonitor monitor, double tPackage,
		...
		List renamedPackages = 
				 detector.detectRefactorings(candidatePackages, monitor, unitOfWorkPerRefactoring);
