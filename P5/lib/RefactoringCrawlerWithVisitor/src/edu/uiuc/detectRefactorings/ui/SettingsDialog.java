...
public class SettingsDialog extends Dialog {
	 ...
	 */
	 ...
	 private Control createSyntacticAnalysisGroup(Composite composite) {
		...
		sPack.setText(" 120");
				 ...
				 useVariableShingles = createCheckbox(group, "UseVariableShingles", true, rowLayouter);
		useVariableShingles
				.setToolTipText("Uses a number of shingles direct proportional with "
				 ...
				 useJavadocComments = createCheckbox(group, "Use Javadoc Comments", false, rowLayouter);
		useJavadocComments
	 ...
	 */
	 private void createProjectSelection(Composite composite) {
		...
		boolean defaultValue = false;
				...
				viewAdvancedSettings = createCheckbox(composite, title, defaultValue, rowLayouter);
	...
	protected void enableAdvancedSettings(boolean enabled) {
	 ...
	 private Control createDetectRefactoringsGroup(Composite composite) {
		...
		defaultValue = false;
				...
				detectPullUpMethod = createCheckbox(group, title, defaultValue, layouter);
		...
		defaultValue = false;
				...
				detectPushDownMethod = createCheckbox(group, title, defaultValue, layouter);
		...
		defaultValue = false;
				 ...
				 detectChangeMethodSignature = createCheckbox(group, title, defaultValue, layouter);
	...
	protected void okPressed() {
		...
		// refactoring settings
				...
				settings.put("RenameMethod", new Boolean(detectRenMethod.getSelection()));
