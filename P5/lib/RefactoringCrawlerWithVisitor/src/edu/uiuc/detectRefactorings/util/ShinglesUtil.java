...
public class ShinglesUtil {
	...
	public int[] computeMethodShingles(String str) {
		return  
						...
						computeMethodShingles(str, w, strategy.upperBoundLimitForShinglesBag(findNumberOfLines(str), s_method));
	}
	...
	public int[] computeMethodShingles(String str, int window,
		...
		int correctNumberOfShingles = 
				Math.min(upperBoundLimit, numberOfWindowedTokens);
	...
	public void initialize(AbstractBaseGraph origGraph,
		...
		verFields = new ArrayList();
				 ...
				 initializeElementsLists(origGraph, origMethods, origClasses, origPackages, origFields);

		   
				initializeElementsLists(verGraph, verMethods, verClasses, verPackages, verFields);
	}
	 ...
	 private void computeClassShingles(List classes, AbstractBaseGraph graph) {
		for (int i = 0; i < classes.size(); i++) {
			...
			Arrays.sort(allShinglesFromMethods);
					...
					upperBoundForClassShingles = Math.min(upperBoundForClassShingles, allShinglesFromMethods.length);
	 ...
	 private void computePackageShingles(List packages, AbstractBaseGraph graph) {
		...
		for (int i = 0; i < packages.size(); i++) {
			...
			if (!pack.getFullyQualifiedName().equals("")) {
				...
				int upperBoundForPackage = 
								 Math.min(strategy
						.upperBoundForPackageShingles(numberOfClasses, s_package), allShinglesFromClasses.length);
